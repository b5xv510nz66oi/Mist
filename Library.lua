...
local Library do 
 ...
 Items["RealSlider"].Instance.ClipsDescendants = true
 ...
 local alpha = if range > 0 then (Slider.Value - Slider.Min) / range else 0
 alpha = MathClamp(alpha, 0, 1)

 Items["Accent"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2New(alpha, 0, 1, 0)})
 Items["Dragger"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2New(alpha, 0, 0.5, 0)})
